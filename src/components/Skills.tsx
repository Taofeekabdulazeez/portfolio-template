...
const skills: Skill[] = [
  { name: 'UI Design', x: 30, y: 30, connections: [1, 2], philosophy: 'Beauty in simplicity' },
  { name: 'Motion', x: 60, y: 20, connections: [2, 3], philosophy: 'Movement tells stories' },
  { name: 'React', x: 50, y: 50, connections: [3, 4], philosophy: 'Building living interfaces' },
  { name: 'Typography', x: 20, y: 60, connections: [0, 4], philosophy: 'Words have weight' },
  { name: 'UX Strategy', x: 70, y: 70, connections: [1], philosophy: 'Empathy drives design' },
  { name: 'Creative Code', x: 40, y: 80, connections: [2], philosophy: 'Code is a creative medium' },
];
    ...
    const drawConnections = () => {
      ...
      skills.forEach((skill, index) => {
        skill.connections.forEach((connIndex) => {
          ...
          const isHighlighted = hoveredSkill === index || hoveredSkill === connIndex;
          ctx.strokeStyle = isHighlighted ? '#8B7355' : 'rgba(245, 243, 239, 0.1)';
              ...
              <div className="relative">
                <div
                  className={`w-4 h-4 rounded-full transition-all duration-300 ${
                    hoveredSkill === index || skill.connections.includes(hoveredSkill ?? -1)
                ...
                <div
                  className={`absolute top-full left-1/2 -translate-x-1/2 mt-4 whitespace-nowrap transition-all duration-300 ${
                    hoveredSkill === index ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                  }`}
                >
